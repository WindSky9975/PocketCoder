...
describe("relay realtime integration", () => {
  it("supports pairing, subscribe, replay, and command forwarding", async () => {
    ...
    const desktopToken = createPairingToken("desktop-1", "2099-01-01T00:05:00.000Z");

    try {
      const tokenResponse = await fetch(
        `${baseUrl}/pairing/token?value=${encodeURIComponent(desktopToken)}`,
      );
      ...
      const desktopSocket = createSocketHarness(`${wsUrl}?deviceId=desktop-1&role=desktop`);
