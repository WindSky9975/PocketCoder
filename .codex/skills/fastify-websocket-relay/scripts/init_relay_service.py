...
TEMPLATE_ROOT = SKILL_ROOT / "assets" / "templates" / "relay"
...
def sync_template_file(
    ...
    destination = repo_root / "apps" / "relay" / template_path.relative_to(TEMPLATE_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder relay service scaffold.",
    )
...
def main() -> int:
    ...
    target_relay_dir = repo_root / "apps" / "relay"
    if args.mode == "init" and target_relay_dir.exists() and any(target_relay_dir.iterdir()):
        print("[warn] apps/relay is not empty; init will only add or compare required scaffold files.")
    if args.mode == "repair" and not target_relay_dir.exists():
        print("[warn] apps/relay does not exist yet; repair will behave like init.")
