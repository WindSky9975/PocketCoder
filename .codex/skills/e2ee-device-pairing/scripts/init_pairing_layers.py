...
TEMPLATE_ROOTS = {
    "agentd-security": SKILL_ROOT / "assets" / "templates" / "agentd-security",
    "relay-security": SKILL_ROOT / "assets" / "templates" / "relay-security",
    "web-crypto": SKILL_ROOT / "assets" / "templates" / "web-crypto",
}
DESTINATION_ROOTS = {
    "agentd-security": Path("apps/agentd"),
    "relay-security": Path("apps/relay"),
    "web-crypto": Path("apps/web/src"),
}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder pairing and E2EE boundary layers.",
    )
...
def main() -> int:
    ...
    for name, relative_target in DESTINATION_ROOTS.items():
        target_dir = repo_root / relative_target
        if args.mode == "init" and target_dir.exists() and any(target_dir.iterdir()):
            print(f"[warn] {relative_target.as_posix()} is not empty; init will only add or compare required pairing files.")
