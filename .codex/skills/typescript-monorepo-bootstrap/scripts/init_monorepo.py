...
PLACEHOLDER_DIRECTORIES = [
    ...
    Path("apps/agentd/tests"),
    Path("apps/relay/src/modules"),
    Path("apps/relay/src/transport/http"),
    Path("apps/relay/src/transport/ws"),
    Path("apps/relay/src/security"),
    Path("apps/relay/src/storage"),
    Path("apps/relay/tests"),
...
def ensure_repo_root(repo_root: Path, result: OperationResult, dry_run: bool) -> None:
    if repo_root.exists():
        if not repo_root.is_dir():
            raise SystemExit(f"Target path is not a directory: {repo_root}")
        return
...
def destination_for_template(template_path: Path) -> Path:
    relative = template_path.relative_to(TEMPLATE_ROOT)

    if relative.parts[0] == "root":
        root_relative = Path(*relative.parts[1:])
        if root_relative == Path(".github-workflows-ci.yml"):
            return Path(".github") / "workflows" / "ci.yml"
        return root_relative

    if relative.parts[0] in {"apps", "packages"}:
        return relative

    raise ValueError(f"Unsupported template path: {relative}")
...
def sync_template_file(
    ...
    destination = repo_root / destination_for_template(template_path)
...
def ensure_placeholder_directory(
    repo_root: Path,
    relative_dir: Path,
    result: OperationResult,
    dry_run: bool,
    ...
    directory_missing = not directory.exists()

    if directory_missing:
        ensure_directory(directory, repo_root, result, dry_run)
        directory_is_empty = True
    else:
        directory_is_empty = not any(directory.iterdir())

    gitkeep = directory / ".gitkeep"
    gitkeep_relative = gitkeep.relative_to(repo_root).as_posix()

    if gitkeep.exists():
        add_unique(result.skipped_files, gitkeep_relative)
        return
    ...
    add_unique(result.created_files, gitkeep_relative)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder monorepo scaffold.",
    )
...
def main() -> int:
    ...
    ensure_repo_root(repo_root, result, args.dry_run)

    if args.mode == "init" and repo_root.exists() and any(repo_root.iterdir()):
        print("[warn] target repository is not empty; init will only add or compare required scaffold files.", file=sys.stderr)
    if args.mode == "repair" and not repo_root.exists():
        print("[warn] target repository does not exist yet; repair will behave like init.", file=sys.stderr)
    ...
    for relative_dir in PLACEHOLDER_DIRECTORIES:
        ensure_placeholder_directory(
            repo_root=repo_root,
