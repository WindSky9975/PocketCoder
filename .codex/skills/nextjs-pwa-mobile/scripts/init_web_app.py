...
TEMPLATE_ROOT = SKILL_ROOT / "assets" / "templates" / "web"
...
def ensure_repo_root(repo_root: Path, dry_run: bool) -> None:
    if repo_root.exists():
...
def destination_for_template(template_path: Path) -> Path:
    return APP_ROOT / template_path.relative_to(TEMPLATE_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder apps/web PWA shell.",
    )
...
def main() -> int:
    ...
    if args.mode == "init" and repo_root.exists() and any(repo_root.iterdir()):
        print(
            "[warn] target repository is not empty; init will only add or compare required web scaffold files.",
            file=sys.stderr,
        )
