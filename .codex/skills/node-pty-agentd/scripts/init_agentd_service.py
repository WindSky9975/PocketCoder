...
TEMPLATE_ROOT = SKILL_ROOT / "assets" / "templates" / "agentd"
...
def sync_template_file(
    ...
    destination = repo_root / "apps" / "agentd" / template_path.relative_to(TEMPLATE_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder agentd service scaffold.",
    )
...
def main() -> int:
    ...
    target_agentd_dir = repo_root / "apps" / "agentd"
    if args.mode == "init" and target_agentd_dir.exists() and any(target_agentd_dir.iterdir()):
        print("[warn] apps/agentd is not empty; init will only add or compare required scaffold files.")
    if args.mode == "repair" and not target_agentd_dir.exists():
        print("[warn] apps/agentd does not exist yet; repair will behave like init.")
