...
TEMPLATE_ROOT = SKILL_ROOT / "assets" / "templates" / "windows-platform"
DESTINATION_ROOT = Path("apps/agentd/src/platform/windows")
...
def sync_template_file(
    ...
    destination = repo_root / DESTINATION_ROOT / template_path.relative_to(TEMPLATE_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder Windows desktop-control boundary.",
    )
...
def main() -> int:
    ...
    target_dir = repo_root / DESTINATION_ROOT
    if args.mode == "init" and target_dir.exists() and any(target_dir.iterdir()):
        print("[warn] apps/agentd/src/platform/windows is not empty; init will only add or compare required platform files.")
    if args.mode == "repair" and not target_dir.exists():
        print("[warn] apps/agentd/src/platform/windows does not exist yet; repair will behave like init.")
