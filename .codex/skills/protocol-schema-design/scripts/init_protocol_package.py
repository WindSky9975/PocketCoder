...
TEMPLATE_ROOT = SKILL_ROOT / "assets" / "templates" / "protocol"
...
def sync_template_file(
    ...
    destination = repo_root / "packages" / "protocol" / template_path.relative_to(TEMPLATE_ROOT)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Initialize or repair the PocketCoder protocol package scaffold.",
    )
...
def main() -> int:
    ...
    target_package_dir = repo_root / "packages" / "protocol"
    if args.mode == "init" and target_package_dir.exists() and any(target_package_dir.iterdir()):
        print("[warn] packages/protocol is not empty; init will only add or compare required scaffold files.")
    if args.mode == "repair" and not target_package_dir.exists():
        print("[warn] packages/protocol does not exist yet; repair will behave like init.")
